...
fn bench_binary_protocol(c: &mut Criterion) {
    ...
    let target = Target::from_bytes([0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 
...
fn bench_retry_logic(c: &mut Criterion) {
    ...
    let errors = vec![
        ...
        Error::Io(std::io::Error::new(std::io::ErrorKind::ConnectionRefused, "Connection refused")),
    ];
...
fn bench_base64_operations(c: &mut Criterion) {
    ...
    group.bench_function("decode_base64", |b| {
        b.iter(|| {
            black_box(base64::engine::general_purpose::STANDARD.decode(base64_string).unwrap());
        });
    });
