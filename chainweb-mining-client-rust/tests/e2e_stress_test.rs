...
impl StressTestCoordinator {
    ...
    async fn stress_test_cpu_mining(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for worker_id in 0..self.config.worker_count {
            ...
            let handle = tokio::spawn(async move {
                            ...
                            if elapsed > 0.0 {
                                ...
                                while current_rate > peak {
                                    match stats.peak_hashrate.compare_exchange_weak(
                                        peak, current_rate, Ordering::Relaxed, Ordering::Relaxed
                                    ) {
                                        Ok(_) => break,
                ...
                stats.total_requests.fetch_add(local_requests, Ordering::Relaxed);
                stats.successful_requests.fetch_add(local_successful, Ordering::Relaxed);
                stats.total_hashes.fetch_add(local_hashes, Ordering::Relaxed);
            });
    ...
    async fn run_cpu_worker(endpoint: &str, account: &str) -> Result<u64, Box<dyn std::error::Error>> {
        let output = Command::new("cargo")
            .args(&["run", "--release", "--", "cpu",
                   "--chainweb-url", endpoint,
                   "--account", account,
                   "--threads", "2",
                   "--batch-size", "10000"])
            .env("RUST_LOG", "info")
    ...
    async fn run_external_worker(endpoint: &str, account: &str) -> Result<(), Box<dyn std::error::Error>> {
        let output = Command::new("cargo")
            .args(&["run", "--release", "--", "external",
                   "--chainweb-url", endpoint,
                   "--account", account,
                   "--command", "echo 'mock-hash'"])
            .output()?;
    ...
    fn print_results(&self) {
        ...
        println!("⏱️  Test Duration:        {} seconds", self.config.test_duration.as_secs());
        println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    }
}
...
async fn test_e2e_cpu_mining_stress() {
    ...
    let coordinator = StressTestCoordinator::new(config).await
        .expect("Failed to create stress test coordinator");
    
    // Run stress test
    coordinator.stress_test_cpu_mining().await
...
async fn test_e2e_stratum_server_stress() {
    ...
    // Run stress test
    coordinator.stress_test_stratum_server().await
...
async fn test_e2e_external_worker_stress() {
    ...
    // Run stress test
    coordinator.stress_test_external_worker().await
...
async fn test_e2e_comprehensive_stress() {
    ...
    let health_monitor = {
        ...
        tokio::spawn(async move {
            coordinator_ref.monitor_node_health().await
        })
    };
