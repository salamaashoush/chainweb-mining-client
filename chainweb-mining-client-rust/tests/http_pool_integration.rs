...
use chainweb_mining_client::protocol::http_pool::{
    ClientType, HttpClientPool, HttpPoolConfig, get_config_client, get_insecure_client,
    get_mining_client, global_http_pool,
};
use std::sync::Arc;
use std::thread;
...
fn test_performance_vs_individual_clients() {
    ...
    assert!(
        pooled_duration <= individual_duration * 2,
...
fn test_error_handling() {
    ...
    for _ in 0..10 {
        ...
        let handle = thread::spawn(move || pool_clone.get_client(ClientType::Mining));
        handles.push(handle);
    }
