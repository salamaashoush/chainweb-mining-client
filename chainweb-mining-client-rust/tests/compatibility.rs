...
use chainweb_mining_client::workers::stratum::{
    Nonce1, Nonce2, NonceSize, compose_nonce, split_nonce,
};
...
fn test_target_comparison_compatibility() {
    // Test that target comparison matches Haskell (little-endian comparison)
    ...
    // Hash that should meet target (smaller in little-endian interpretation)
    let good_hash = [
        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
        0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
        0xFF, 0x0E,
    ...
    // Hash that should not meet target (larger in little-endian interpretation)
    let bad_hash = [
...
fn test_stratum_protocol_compatibility() {
    ...
    use chainweb_mining_client::workers::stratum::{decode_hex, encode_hex};
...
fn test_preemption_strategy_compatibility() {
    ...
    use chainweb_mining_client::core::{PreemptionConfig, PreemptionStrategy, WorkPreemptor};
...
fn test_blockchain_header_compatibility() {
    ...
    const _: () = assert!(TARGET_OFFSET + TARGET_SIZE <= WORK_SIZE);
