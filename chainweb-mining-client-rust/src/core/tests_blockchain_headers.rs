...
use crate::core::{Target, Work, constants::WORK_SIZE};
use std::fs;
...
fn load_test_headers() -> Vec<Vec<u8>> {
    ...
    if offset < bytes.len() {
        panic!(
            "Incomplete header at end of file. Expected {} bytes, got {}",
            HEADER_SIZE,
            bytes.len() - offset
        );
    }
    ...
    fn test_target_extraction() {
        ...
        if !works.is_empty() {
            ...
            assert_ne!(
                target.as_bytes(),
                &[0u8; 32],
                "Target should not be all zeros"
            );
        }
    }
